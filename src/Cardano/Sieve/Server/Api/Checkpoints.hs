...
module Cardano.Sieve.Server.Api.Checkpoints
  ( CheckpointsAPI
  , checkpointsServer
  )
where
...
import Cardano.Sieve.Server.Api.Common (hexText, withReadConnection)
...
-- | Chain points the indexer has recorded.
--
...
-- point is @null@, not a 404, matching \/datums.
type CheckpointsAPI =
...
-- | How @GET \/checkpoints\/{slot-no}@ matches the requested slot.
...
-- | A chain point in its wire shape.
